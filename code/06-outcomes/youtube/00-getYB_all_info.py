...
# Build deduplicated mapping from source Excel
mapping_rows = []
...
print(f"  {len(df_map):,} name-channel pairs from source Excel")

# Merge with manually-corrected mapping file (add any rows missing from auto-parsed)
os.makedirs(OUT_FOLDER, exist_ok=True)
map_path = os.path.join(OUT_FOLDER, "name_youtube_mapping.xlsx")
if os.path.exists(map_path):
    df_manual = pd.read_excel(map_path)
    df_manual = df_manual[df_manual["yt_handle"].notna() & (df_manual["yt_handle"] != "")]
    df_map = pd.concat([df_map, df_manual], ignore_index=True)
    df_map = df_map.drop_duplicates(subset=["name.page", "yt_handle"]).reset_index(drop=True)
    print(f"  {len(df_map):,} name-channel pairs after merging manual corrections")
...
# Skip handles already present in the existing output
out_path = os.path.join(OUT_FOLDER, "mexican_newspapers_youtube.xlsx")
if os.path.exists(out_path):
    df_existing = pd.read_excel(out_path)
    already_done = set(df_existing.loc[df_existing["channelId"].notna(), "yt_handle"].dropna().unique())
else:
    df_existing = pd.DataFrame()
    already_done = set()

unique_handles = [h for h in df_map["yt_handle"].unique().tolist() if h not in already_done]
print(f"\nFetching info for {len(unique_handles):,} new handles (skipping {len(already_done):,} already fetched) ...")
...
for i, handle in enumerate(unique_handles, 1):
    ...
    try:
        ...
        # Fallback: some older channels have a legacy username instead of a @handle
        if not items:
            resp = call_with_retries(lambda h=handle: youtube.channels().list(
                part="id,snippet,statistics,contentDetails,brandingSettings,topicDetails",
                forUsername=h,
...
df_channels = pd.DataFrame(channel_rows) if channel_rows else pd.DataFrame(columns=["yt_handle"])
...
df_new_out = df_map[df_map["yt_handle"].isin(unique_handles)].merge(df_channels, on="yt_handle", how="left")

# Append new rows to existing and save (drop old rows that had no channel data)
df_existing = df_existing[df_existing["channelId"].notna()] if "channelId" in df_existing.columns else df_existing
df_out = pd.concat([df_existing, df_new_out], ignore_index=True)
df_out = df_out.drop_duplicates(subset=["name.page", "yt_handle"]).reset_index(drop=True)

df_out.to_excel(out_path, index=False)

print(f"\nSaved: {out_path}")
print(f"   Rows: {len(df_out):,}  |  Columns: {len(df_out.columns):,}")
if not df_new_out.empty:
    preview_cols = [c for c in ["name.page", "yt_handle", "yt_title", "yt_subscriberCount", "yt_videoCount"] if c in df_new_out.columns]
    print(df_new_out[preview_cols].head(10).to_string())
else:
    print("  (no new rows to preview)")
