...
CHANNELS = [
    {"channel_id": "UCFxHplbcoJK9m70c4VyTIxg", "handle": "milenio"},
    # Add more channels below:
    {"channel_id": "UCUsm-fannqOY02PNN67C0KA", "handle": "nmas"},
    {"channel_id": "UC2SEk1w7DgoHs_G3w1biz-w", "handle": "eluniversalmex"},
    {"channel_id": "UCUP6qv-_EIL0hwTsJaKYnvw", "handle": "aztecanoticias"},
    {"channel_id": "UCzmd9Aj2wmPggWZJpLdBB8Q", "handle": "grupoformula-rf"},
    {"channel_id": "UC7k--FhnJzhPTrbtldMSoQQ", "handle": "adnnoticiasmx"},
    {"channel_id": "UCvJS-YNyaWyOucx8bGrHVvw", "handle": "efeagenciaefe"}
]

OUT_BASE = media_output_path("data", "06-outcomes", "youtube", "playlists", ".keep").parent
# Outputs per channel:
#   playlists/videos/{handle}.parquet       ← globbed by 04-build_panel.py
#   playlists/playlists/{handle}.parquet    ← playlist metadata
#   playlists/checkpoints/{handle}.json     ← per-channel resume checkpoint
...
# =========================
# Checkpoint (per-channel)
# =========================

def ckpt_path(handle):
    return os.path.join(OUT_BASE, "checkpoints", f"{handle}.json")

def load_checkpoint(handle):
    p = ckpt_path(handle)
    if os.path.exists(p):
...
def save_checkpoint(handle, state):
    with open(ckpt_path(handle), "w", encoding="utf-8") as f:
        json.dump(state, f, ensure_ascii=False)

def clear_checkpoint(handle):
    p = ckpt_path(handle)
    if os.path.exists(p):
        os.remove(p)
...
os.makedirs(os.path.join(OUT_BASE, "videos"),      exist_ok=True)
os.makedirs(os.path.join(OUT_BASE, "playlists"),   exist_ok=True)
...
for ch in CHANNELS:
    ...
    handle     = ch["handle"]
    print(f"\n{'=' * 60}")
    print(f"Processing: {handle}  ({channel_id})")
    print(f"{'=' * 60}")

    playlists_path = os.path.join(OUT_BASE, "playlists", f"{handle}.parquet")
    videos_path    = os.path.join(OUT_BASE, "videos",    f"{handle}.parquet")

    # Skip if videos already complete (no checkpoint in progress)
    if os.path.exists(videos_path) and not os.path.exists(ckpt_path(handle)):
        print(f"  Already complete — skipping ({videos_path})")
        continue

    state = load_checkpoint(handle)
    ...
    if state["playlists_done"] and os.path.exists(playlists_path):
        ...
        print(f"  Playlists already saved ({len(df_playlists):,}) — skipping fetch.")
    else:
    ...
    print(f"  Collecting video IDs: {len(remaining):,} playlists remaining "
    ...
    print(f"  Done. {len(df_videos):,} videos → {videos_path}")
