...
# ---- setup ----
# Standalone export script: outputs PDF figures and tables to one results folder.
...
RESULTS_DIR <- dirname(media_output_path('results', 'descriptive_stats', '.keep'))
...
save_plot <- function(p, filename, w = 12, h = 7) {
  save_plot_to(p, filename, RESULTS_DIR, w = w, h = h, dpi = 150)
}

latex_dims <- function(w, h, max_w = 10, max_h = 7.5) {
  scale_factor <- min(max_w / w, max_h / h, 1)
  c(width = w * scale_factor, height = h * scale_factor)
}

save_plot_to <- function(p, filename, out_dir, w = 12, h = 7, dpi = 300, bg = "white") {
  dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)

  pdf_path <- file.path(out_dir, paste0(tools::file_path_sans_ext(filename), ".pdf"))
  dims <- latex_dims(w, h)
...
save_table_pdf <- function(data, stem, title = NULL, out_dir = RESULTS_DIR,
                           width = 11, base_height = 1.2, row_height = 0.28) {
  dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)

  pdf_path <- file.path(out_dir, paste0(stem, ".pdf"))
  height <- max(4, min(14, base_height + row_height * (nrow(data) + 2)))
  ...
  title_grob <- if (is.null(title)) {
    NULL
  } else {
    grid::textGrob(title, gp = grid::gpar(fontface = "bold", cex = 1))
  }

  grDevices::cairo_pdf(pdf_path, width = width, height = height)
  grid::grid.newpage()
  grid::grid.draw(gridExtra::arrangeGrob(table_grob, top = title_grob))
  grDevices::dev.off()
...
p1a <- ggplot(conf_daily, aes(fecha, n_total_attend)) +
  ...
  labs(title = "Daily Total Attendance at Mananeras",
...
save_table_pdf(
  ...
  title = "Descriptive Statistics for Panel Variables"
)
...
p_total_spending <- ggplot(df_c_agg_yearly, aes(x = year.gasto, y = costo)) +
...
save_plot(p_total_spending, "00_yearly_spending.png", w = 10, h = 6)
...
p_t200_dist <- ggplot(df_c_pos_flag, aes(costo, fill = group)) +
  ...
  labs(title = "Contract-Size Distribution: Top 200 vs Rest (log10 scale)",
...
save_fig <- function(p, name, w = 12, h = 7) {
  save_plot_to(p, name, RESULTS_DIR, w = w, h = h, dpi = 300, bg = "white")
}
...
yt_ch_summary_tbl <- fmt_tbl(yt_ch_summary)

save_table_pdf(
  yt_ch_summary_tbl,
  "yt_channel_totals_summary",
  title = "Table A: Channel-Level Totals (2018-2024)"
)

yt_ch_summary_tbl

# ---- yt-summary-table-pv ----
yt_pv_summary_tbl <- fmt_tbl(yt_pv_summary)

save_table_pdf(
  yt_pv_summary_tbl,
  "yt_per_video_summary",
  title = "Table B: Per-Video Metrics (Active Days Only)"
)
