"""Export RSS article rows into per-source parquet.gzip files."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Write per-source RSS article parquet.gzip part files from SQLite.")
...
def iter_sqlite_chunks(db_path: Path, chunksize: int):
    """Yield chunks from the RSS article SQLite table."""
    import sqlite3
...
def list_sources(db_path: Path, source_filter: set[str] | None) -> None:
    """Print source row counts from the RSS article SQLite table."""
    import sqlite3
