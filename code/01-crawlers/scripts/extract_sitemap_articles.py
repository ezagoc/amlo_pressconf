"""Extract article text from sitemap-discovered HTML pages."""
...
DEFAULT_DISCOVERY = media_input_path(
    ...
    "crawler",
    "discovery",
    "discovered_urls_sitemap.parquet",
)
DEFAULT_ARTICLES_PARTS = ("data", "00-newspaper_data", "crawler", "articles_sitemap", ".keep")
DEFAULT_REPORTS_PARTS = ("data", "00-newspaper_data", "crawler", "reports", ".keep")
DEFAULT_STATE_DB_NAME = "sitemap_articles.sqlite"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract full article text from sitemap-discovered HTML pages."
    )
    ...
    parser.add_argument(
        ...
        default="default",
        help="HTTP request profile. Use browser for sites that reject the default research-bot user agent.",
    )
...
def main() -> None:
    ...
    if args.resume:
        ...
        if existing is None:
            ...
            print(f"Resume enabled: loaded {len(existing):,} existing sitemap article status rows")
