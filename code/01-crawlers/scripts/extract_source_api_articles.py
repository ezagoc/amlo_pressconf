"""Extract article text from source-API-discovered URLs."""
...
DEFAULT_DISCOVERY = media_path(
    ...
    "crawler",
    "discovery",
    "discovered_urls_source_api.parquet",
)
...
DEFAULT_STATE_DB_NAME = "source_api_articles.sqlite"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract full article text from source-API-discovered HTML pages."
    )
    ...
    parser.add_argument(
        ...
        default="browser",
        help="HTTP request profile. Source API extraction defaults to browser-like headers.",
    )
...
def main() -> None:
    ...
    if args.resume:
        ...
        if existing is None:
            print("Resume enabled: no existing source API SQLite progress DB found")
        else:
            print(f"Resume enabled: loaded {len(existing):,} existing source API article status rows")
