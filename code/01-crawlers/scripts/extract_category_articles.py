"""Extract article text from category-discovered URLs."""
...
from project_paths import media_path, media_output_path  # noqa: E402
...
DEFAULT_DISCOVERY = media_path(
    ...
    "crawler",
    "discovery",
    "discovered_urls_category.parquet",
)
DEFAULT_ARTICLES_PARTS = ("data", "00-newspaper_data", "crawler", "articles_category", ".keep")
DEFAULT_REPORTS_PARTS = ("data", "00-newspaper_data", "crawler", "reports", ".keep")
DEFAULT_STATE_DB_NAME = "category_articles.sqlite"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract full article text from category-discovered HTML pages."
    )
    ...
    parser.add_argument(
        "--refresh-date-after",
        default=None,
        help="Re-extract existing rows whose saved article date parses after this cutoff, e.g. 2026-09-07.",
    )
    ...
    parser.add_argument(
        ...
        default="browser",
        help="HTTP request profile. Category extraction defaults to browser-like headers.",
    )
...
def main() -> None:
    ...
    if args.resume:
        ...
        if existing is None:
            print("Resume enabled: no existing category SQLite progress DB found")
        else:
            print(f"Resume enabled: loaded {len(existing):,} existing category article status rows")
    ...
    queue = prepare_sitemap_article_queue(
        ...
        refresh_date_after=args.refresh_date_after,
