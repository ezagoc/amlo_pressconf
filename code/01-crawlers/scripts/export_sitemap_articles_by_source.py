"""Export sitemap HTML article rows into per-source parquet.gzip files."""
...
DEFAULT_STATE_DB_PARTS = (
    ...
    "crawler",
    "articles_sitemap",
    "sitemap_articles.sqlite",
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Write per-source sitemap article parquet.gzip part files from SQLite."
    )
...
def main() -> None:
    ...
    for chunk_number, chunk in enumerate(iter_sqlite_chunks(state_db, args.chunksize), start=1):
        chunk = chunk[
            chunk.apply(
                lambda row: likely_article_url_for_source(row.get("source_id"), row.get("url")),
                axis=1,
            )
        ].copy()
...
def iter_sqlite_chunks(db_path: Path, chunksize: int):
    """Yield chunks from the sitemap article SQLite table."""
    import sqlite3
...
def list_sources(db_path: Path, source_filter: set[str] | None) -> None:
    """Print source row counts from sitemap article SQLite."""
    import sqlite3
