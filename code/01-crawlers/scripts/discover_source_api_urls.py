"""Discover article URLs through source-specific API/listing adapters."""
...
from crawler_core.source_api import (  # noqa: E402
    build_source_api_report,
    dedupe_source_api_discovery,
    discover_source_api_urls,
    load_source_api_sources,
    write_source_api_outputs,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Discover article URLs through source-specific API/listing adapters."
    )
    ...
    parser.add_argument("--max-pages-per-section", type=int, default=100)
    ...
    parser.add_argument(
        ...
        default="browser",
        help="HTTP request profile. Source API discovery defaults to browser-like headers.",
    )
    parser.add_argument(
        "--append-existing",
        action="store_true",
        help=(
            "Merge new rows into discovered_urls_source_api.*. When --source-id "
...
def main() -> None:
    ...
    sources = load_source_api_sources(args.capabilities, source_ids=args.source_id)
    discovered = discover_source_api_urls(
        sources,
    ...
    report = build_source_api_report(discovered)
    outputs = write_source_api_outputs(discovered, report, discovery_dir, reports_dir)

    print(f"Source API sources considered: {len(sources):,}")
...
def merge_existing_discovery(
    ...
    *,
    ...
    """Merge new source API rows with an existing discovery output."""
    parquet_path = discovery_dir / "discovered_urls_source_api.parquet"
    csv_path = discovery_dir / "discovered_urls_source_api.csv"
    ...
    return dedupe_source_api_discovery(merged)
