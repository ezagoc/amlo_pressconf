"""Extract article text from Wayback-discovered archived URLs."""
...
DEFAULT_DISCOVERY = media_path(
    ...
    "crawler",
    "discovery",
    "discovered_urls_wayback.parquet",
)
DEFAULT_ARTICLES_PARTS = ("data", "00-newspaper_data", "crawler", "articles_wayback", ".keep")
DEFAULT_REPORTS_PARTS = ("data", "00-newspaper_data", "crawler", "reports", ".keep")
DEFAULT_STATE_DB_NAME = "wayback_articles.sqlite"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract full article text from Wayback archived HTML pages."
    )
    ...
    parser.add_argument(
        ...
        default="browser",
        help="HTTP request profile. Wayback extraction defaults to browser-like headers.",
    )
...
def main() -> None:
    ...
    if args.resume:
        ...
        if existing is None:
            ...
            print(f"Resume enabled: loaded {len(existing):,} existing Wayback article status rows")
