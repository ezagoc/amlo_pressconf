"""Export Common Crawl article rows into per-source parquet.gzip files."""
...
DEFAULT_STATE_DB_PARTS = (
    ...
    "00-newspaper_data",
    "crawler",
    "articles_commoncrawl",
    "commoncrawl_articles.sqlite",
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Write per-source Common Crawl article parquet.gzip part files from SQLite."
    )
...
def iter_sqlite_chunks(db_path: Path, chunksize: int):
    """Yield chunks from the Common Crawl article SQLite table."""
    import sqlite3
...
def list_sources(db_path: Path, source_filter: set[str] | None) -> None:
    """Print source row counts from the Common Crawl article SQLite table."""
    import sqlite3
