"""Extract article text from Common Crawl-discovered URLs."""
...
DEFAULT_DISCOVERY = media_path(
    ...
    "crawler",
    "discovery",
    "discovered_urls_commoncrawl.parquet",
)
DEFAULT_ARTICLES_PARTS = ("data", "00-newspaper_data", "crawler", "articles_commoncrawl", ".keep")
DEFAULT_REPORTS_PARTS = ("data", "00-newspaper_data", "crawler", "reports", ".keep")
DEFAULT_STATE_DB_NAME = "commoncrawl_articles.sqlite"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract full article text from Common Crawl-discovered live URLs."
    )
    ...
    parser.add_argument(
        ...
        default="browser",
        help="HTTP request profile. Common Crawl extraction defaults to browser-like headers.",
    )
...
def main() -> None:
    ...
    if args.resume:
        ...
        if existing is None:
            print("Resume enabled: no existing Common Crawl SQLite progress DB found")
        else:
            print(f"Resume enabled: loaded {len(existing):,} existing Common Crawl article status rows")
