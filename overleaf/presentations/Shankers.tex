...
\begin{frame}
\frametitle{Literature Review, Szeidl and Szucs (2021)}

\small
\begin{itemize}
\item Does media ideology and favor exchange play a part? YES
\item In the context of Hungary,  \cite{MediaCapture} show using reduced form evidence and a structural model that: 

\begin{enumerate}
    \item The government allocates substantial advertising resources to politically connected media outlets.
    \item Connected media provide favorable corruption-related coverage to the government.
    \item Both, media owner ideology and explicit favor exchange matter. 
    \item Market based explanations are ruled out (changes in advertisers target audiences)
\end{enumerate}
...
\small
\begin{itemize}
\item Could there be a deterrence effect? A punishment one?
\item \cite{MediaCapture} also find that owner ideology can be shifted by government advertising.
\item The mechanism is clear: ad spending increases the profitability of pro-government ownership.

\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Models, Besley and Prat (2006)}

\small
\begin{itemize}
\item They investigate the contexts in which media capture is more probable in democracies. 
\item For them, media capture is related with the ability of the government to give out rents (bribes in their context, could be ad spending in mine) and the overall media ecosystem.
\item Three main findings: 
\begin{itemize}
    \item Media pluralism provides effective protection against capture (commercial revenue of the unpaid outlets go up).
    %  Every time the government pays an outlet to suppress its information, the commercial revenue of the other outlets goes up because they face less competition on the commercial side. If the government wants to buy out all the media, it has to pay each of them as if it were a monopoly provider of unbiased information.
    \item The degree of independence of the media is given by the difficulty with which the state is able to transfer resources to the media.
    \item Media capture affects political outcomes. 
\end{itemize}

\end{itemize}
\end{frame}

\begin{frame}{Models, Gehlbach and Sonin (2014)}
\small
\begin{itemize}
    \item Non-democratic context. State and private owned media.
    \item Media bias is constrained by the fact that excessive misreporting reduces viewership and therefore, advertising revenue. 
    \item Preferences for citizen mobilization increase bias: 
    \begin{itemize}
        \item Mobilizing is a costly political action.
        \item Governments that prefer it are willing to invest a lot to achieve it.
    \end{itemize}
    \item Larger advertising markets reduce bias. 
    \item A bigger advertising market makes viewership more valuable, so the cost of inducing bias (by losing viewers) becomes higher.
\end{itemize}
\end{frame}
...
\small
\begin{itemize}
    \item What we hope are contributions are going to be. 
    \item From the \cite{MediaCapture} paper I think we distinguish ourselves on several things: 
    \begin{itemize}
        \item We will analyze the whole set of media outlets, through repeated interactions, instead of just 5 across manually detected corruption scandals. 
        \item Hopefully, our identification for the reduced form would be better (given randomization)
        \item Our model (if we include it in that paper) would try to capture the dynamic interaction between media and government.
    \end{itemize}
    \item The other two models I think will benefit us to set up ours, and we base part of our model on their results.
\end{itemize}
